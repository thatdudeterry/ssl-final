...
API.fetchPokemon = async () => {
	...
	console.log("This is showing entire array --->", response);
};

API.findPokemon = async (term) => {
	const response = await axios.get(`${API_BASE_URL}?name=${term}`, term);
...
API.getPokemonByName = async (pokemon) => {
	try {
		//^ in console, this shows as the whole array, is not filtering
